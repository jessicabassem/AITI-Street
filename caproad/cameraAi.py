...
ALIASES = {
    "arduino_car": [
        ...
        "arduino project car",
        "arduino robotic vehicle",
        "diy arduino rc car",
        "homemade arduino rc car",
        "custom arduino rc car",
        "arduino car chassis",
        "arduino robot car chassis",
        "arduino car with l298n",
        "arduino car motor driver",
        "arduino car with motor driver board",
        "rc car with arduino controller",
        "arduino car with breadboard",
        "arduino rc car with exposed wiring",
        "arduino car with cables visible",
        "arduino car with battery pack",
        "arduino project vehicle with electronics visible",
        "arduino robotic car",
        "arduino 2wd robot car",
        "arduino 4wd robot car",
        ...
        "arduino car with ultrasonic sensor",
        "arduino rc car with hc-sr04",
        "robot car with acrylic chassis",
        "robot car with yellow motors",
        "open-frame arduino rc car",
        "mini arduino robotic vehicle",
    ],

    # OPTIONAL: add emergency prompts if you want (any frame true => emergency true over WINDOW)
    # "emergency_vehicle": [
    #     "toy ambulance",
    #     "toy police car",
    #     "toy fire truck",
    ...
    while True:
        ...
        if len(set(counts_in_order)) == 1:
            ...
            if len(roads_with_max) != 1:
                ...
                max_road = roads_with_max[0] + 1  # 1..4
                mode_to_send = 1 if max_road in (1, 3) else 2

        # Optional emergency override (only meaningful if you have emergency prompts)
        # If you want a dedicated emergency mode, define it in Arduino and uncomment:
        # if agg_emergency:
        #     mode_to_send = 3
        ...
        print(f"[FRAME {frame_idx}] counts={road_counts} | agg={agg_counts} | emergency={agg_emergency} | mode={mode_to_send}")
